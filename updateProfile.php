...
if(isset($_POST['firstname'])){
    if($fname!=null){
        ...
        //prepared
        $stmt1 = $db->prepare("UPDATE users SET ufname=? WHERE uusername=?");
...
if(isset($_POST['lastname'])){
    if($lname!=null) {
        $stmt1 = $db->prepare("UPDATE users SET ulname=? WHERE uusername=?");
...
if(isset($_POST['email'])){
    if($email!=null) {
        $stmt1 = $db->prepare("UPDATE users SET uemail=? WHERE uusername=?");
...
if(isset($_POST['psw'])){
    if($pass!=null) {
        $stmt1 = $db->prepare("UPDATE users SET upassword=? WHERE uusername=?");
