...
if(count($base)==1){
    ...
    print_r($matches_made);
        ...
        if($count==0) { ///testing here
            ...
            //simple potential match ----------------------2
            $sql = "SELECT uid,uusername from matches_final where (uage BETWEEN $splitString[0] AND $splitString[1]) AND  not uid in ($uid) and usex='$sex_matrix' and searchBaseCode LIKE '$scode' or searchTypeCode LIKE '$stype'  and uusername not in ( '" . implode($matches_made, "', '") . "' )  Limit 1";
            $result = $db->query($sql);
            while ($row = $result->fetch_array()) {
                echo '<br>';
                echo "UID: " . $row["uid"];
